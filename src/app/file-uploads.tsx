...
export type UploadTask = {
  ...
  conflict?: {
    ...
    resolve: (choice: { mode: 'replace' | 'rename' | 'skip'; name?: string; all?: boolean }) => void
  }
  destination: string
  name: string
  count: number
...
export const uploadActive = (task: UploadTask) =>
...
export const fileTaskTitle = (task: UploadTask) =>
  task.kind && task.kind !== 'upload' ? tr('fileTasks.' + task.kind) : tr('uploads.title')
const taskIcon = (task: UploadTask) =>
  task.status === 'waiting'
    ? mdiAlertCircleOutline
    : {
        copy: mdiContentCopy,
...
export function FileUploadTasks() {
  ...
  return (
    <div className="upload-tasks">
      {tasks.map((task) => (
        <article className="upload-task" key={task.id}>
          <div className="upload-task-heading">
            ...
            <span className={task.status === 'failed' || task.status === 'waiting' ? 'text-error' : 'muted'}>
              {tr(`uploads.${task.status}`)}
            </span>
            {task.conflict && (
              <Button title={tr('fileTasks.resolve')} onClick={() => setResolving(task.id)}>
            ...
            <Button
              ...
              title={tr(uploadActive(task) ? 'fileTasks.cancel' : 'uploads.dismiss')}
              onClick={() => {
          ...
          {uploadActive(task) && (
            <>
              ...
              <progress max={100} value={uploadPercent(task)} aria-label={fileTaskTitle(task)} />
            </>
          )}
          <small>
            {task.completed}/{task.count}
            {(task.skipped ?? 0) > 0 ? ' · ' + tr('fileTasks.skipped', { count: task.skipped }) : ''}
            {(!task.kind || task.kind === 'upload') && (
              <>
                {' '}
      ...
      {selected?.conflict && (
        <ConflictDialog
          key={selected.id + selected.conflict.destination}
          task={selected}
  ...
  return (
    ...
    >
      <Dialog.Portal>
        ...
        <DialogContent
          className="dialog file-conflict-dialog"
          header={
            <>
              {' '}
          ...
          footer={
            <div className="actions">
              <Button disabled={!conflict.replaceAllowed} onClick={() => resolve('replace')}>
                {tr('fileTasks.replace')}
              </Button>
              <Button disabled={!valid} onClick={() => resolve('rename')}>
                {tr('fileTasks.rename')}
              </Button>
        ...
        >
          <p className="file-destination-path">
            <strong>{conflict.destination}</strong>
          </p>
