...
export function AvatarSettings() {
  ...
  const save = useMutation({
    mutationFn: async (file: File | null) => {
      const r = await fetch('/api/v1/avatar', {
  ...
  return (
    <section className="surface">
      <WaitingSurface busy={save.isPending}>
        <h2>{tr('accounts.avatar')}</h2>
        {data.data?.version && (
          <img
            className="profile-avatar-image"
            src={`/api/v1/avatar/image?v=${data.data.version}`}
            alt={tr('accounts.avatar')}
          />
        )}
        <input
          type="file"
        ...
        <div className="actions">
          <Button
            title={tr('accounts.uploadAvatar')}
          ...
          <Button
            ...
            aria-label={tr('accounts.removeAvatar')}
            disabled={!data.data?.version}
