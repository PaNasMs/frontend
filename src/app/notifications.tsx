...
import { mdiTextBoxSearchOutline, mdiClose, mdiArrowRight, mdiInformationOutline, mdiCheckCircleOutline, mdiAlertOutline, mdiAlertCircleOutline } from '@mdi/js'
...
const severityIcons = {info: mdiInformationOutline, success: mdiCheckCircleOutline, warning: mdiAlertOutline, error: mdiAlertCircleOutline}
function Importance({alert}: {alert: Alert}) {
  const severity = alert.severity ?? 'info'
  return <span className={'notification-severity '+severity} role="img" aria-label={tr('alerts.severity.'+severity)} title={tr('alerts.severity.'+severity)}><Icon path={severityIcons[severity]}/></span>
}
let toastSequence = 0
    ...
    <>
      ...
      {data.data?.map((a) => (
        <article className="surface notification-item" key={a.id}>
          <div className="notification-message"><Importance alert={a}/><p>{a.message}</p></div>
          {a.active && !a.id.startsWith('job:') && <p className="small muted">{tr('alerts.problemHint')}</p>}
          <div className="notification-footer">
          <time className="small muted" dateTime={a.updated}>{new Date(a.updated).toLocaleString(locale())}</time>
